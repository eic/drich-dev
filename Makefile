...
EICRECON_DIR = ${DRICH_DEV}/EICrecon/src/services/geometry/irt
DEPS += -I$(EICRECON_DIR)

LIB_TARGET = lib
IRTGEO_LIB_NAME = IrtGeo
IRTGEO_LIB = $(LIB_TARGET)/lib$(IRTGEO_LIB_NAME).so
IRTGEO_ROOT = $(EICRECON_DIR)/irtgeo
IRTGEO_SOURCES := $(wildcard $(IRTGEO_ROOT)/*.cc)
IRTGEO_HEADERS := $(wildcard $(IRTGEO_ROOT)/*.h)
