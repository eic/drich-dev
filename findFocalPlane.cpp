...
int main(int argc, char** argv) {
  ...
  df1.Foreach(
	      [d,e,fp,nphotons](RVec<bool> refl, RVec<TVector3> dir, RVec<TVector3> end){	      
		if( refl[0]  )  d.push_back(dir[0]); e.push_back(end[0]);
		nphotons++;
		if(nphotons==10){
  ...
  for(int i = 0 ; i < fp.size(); i++){
    if( std::abs(fp[i].X()) < 1000 && std::abs(fp[i].Y()) < 1000 && std::abs(fp[i].Z()) < 1000){
      fprintf(outtxt, "%lf %lf %lf \n", fp[i].X(), fp[i].Y(), fp[i].Z());
    }
  }
  fclose(outtxt);
